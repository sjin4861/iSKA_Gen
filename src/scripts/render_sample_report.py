#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""생성(JSONL) + 평가(JSONL) 합쳐 HTML 리포트 렌더링 (평가 표 1장)
...
def _score_badge(score: Optional[int], rubric_id: str) -> str:
    ...
    if rubric_id in binary:
        return f"<span class='badge {'badge-pass' if score==1 else 'badge-fail'}'>{'PASS' if score==1 else 'FAIL'}</span>"
    # Likert: l2_learner_suitability 등
    return f"<span class='badge badge-num'>{score}</span>"
...
def _render_evaluation_table(eval_records: List[Dict[str, Any]]) -> str:
    ...
    for rec in eval_records:
        ...
        # Scope: R1은 all-stems, 나머지는 Sample
        if rid == "completeness_for_guidelines":
            ...
            # R2~R6는 sample 레벨로만 들어오도록 평가 스크립트에서 생성됨
            if stem_idx != -1:
                # 혹시라도 stem 단위가 들어오면 스킵
                continue
            scope = "Sample"
...
# -------------------------
def parse_args():
    p = argparse.ArgumentParser(description="생성+평가 HTML 리포트 생성 (단일 표 렌더)")
...
def main():
    ...
    # eval 경로 결정: --eval > (없으면) sample과 같은 폴더의 *_eval.jsonl
    if args.eval:
