...
from src.domain.repositories.audio_repository import AudioRepository
...
class AudioRepositoryImpl(AudioRepository):
    """
    - find(): Raw_Output에서 audio_script 후보를 로드
    ...
    def find(self, query: OutputQuery) -> Iterable[CandidateOutput]:
        return self.raw_output.find_candidates(ContentType.audio_script, query)
    ...
    def generate_one(
        ...
        # prompt = None
        # if template_key == "passage_agent.create_dialogue_passage":
        #     prompt = self.templates.get(
        #         template_key,
        #         problem_type1=problem_types[0],
        #         eval_goal1=eval_goals[0],
    ...
    def generate_and_fill_missing(
        ...
        for s in sources:
            ...
            c = CandidateOutput(
                ...
                content_type=ContentType.audio_script,
                content=text,
