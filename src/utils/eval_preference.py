...
def evaluate_gpt4_preference_alignment(
    ...
    try:
        ...
        for i, pair in enumerate(pairwise_data, 1):
            ...
            # 프롬프트 템플릿을 인자와 함께 가져오기
            prompt = get_prompt("preference_evaluation.evaluation_prompt", agent="iska", text_a=text_a, text_b=text_b)
            
            try:
                response = client.call([{"role": "user", "content": prompt}])
                
                # GPT-4o 응답에서 선택 추출 (새로운 형식: "A" 또는 "B"로 시작)
                gpt_choice = None
...
def run_preference_ranking_evaluation(
    ...
    try:
        ...
        # pairwise_data를 compare_passage_pairs_with_reward_model에 적합한 형태로 변환
        comparison_pairs = []
        for pair_data in pairwise_data:
            comparison_pairs.append({
                "chosen": pair_data["chosen"],
                "rejected": pair_data["rejected"]
            })
        
        rm_results = reward_model.batch_compare(comparison_pairs, max_length)
        
        # 결과에 pair_id와 추가 정보 매핑
        detailed_results = []
        for i, (pair_data, comparison) in enumerate(zip(pairwise_data, rm_results.get("comparisons", []))):
            detailed_results.append({
                "pair_id": pair_data["pair_id"],
                "topic": pair_data["topic"],
                "prefers_chosen": comparison.get("correct_preference", False),
        ...
        rm_results["detailed_results"] = detailed_results
...
if __name__ == "__main__":
    ...
    result = run_preference_ranking_evaluation(
        reward_model_path=os.path.expanduser("~/models/train_2025-07-26-12-04-23"),  # 틸드 확장
        benchmark_file="v1/iSKA-Gen_Benchmark_v1.0.0_20250726_PreferenceRanking.json",
        openai_model="gpt-4o",
        max_length=256,
        device="auto",
        gpus=None  # 필요시 [0, 1] 등으로 설정
    )
